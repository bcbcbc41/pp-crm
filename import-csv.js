...
if (!csvFile) {
  console.error('Usage: node import-csv.js <fichier.csv>');
  process.exit(1);
}
...
// Parse le CSV (séparateur point-virgule, format date DD.MM.YYYY)
const content = fs.readFileSync(csvFile, 'utf-8').replace(/^\uFEFF/, ''); // retire BOM
